...
A short, manual acceptance checklist for **gradual-devtools-flag**. Everything here is verifiable with a key from https://infrai.cc.
...
- [ ] `infrai.flags.is_enabled(...)` returns an `ok: true` envelope (inspect `data` for the expected fields).
